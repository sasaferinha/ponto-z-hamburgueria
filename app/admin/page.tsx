...
export default function AdminPage() {
  ...
  const printOrder = (order: Order) => {
    ...
    if (!popup) return;
    const rows = items.map((item) => `<div class="item"><b>${item.quantity}x ${item.name}</b><span>${money(Math.round((item.price + item.addOns.reduce((s, a) => s + a.price, 0)) * item.quantity * 100))}</span></div>${item.addOns.map((a) => `<small>+ ${a.name} — ${money(Math.round(a.price * 100))}${item.quantity > 1 ? " cada" : ""}</small>`).join("")}`).join("");
