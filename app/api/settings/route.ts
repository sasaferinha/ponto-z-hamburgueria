...
const defaults = { id: 1, isOpen: true, deliveryTime: "40 a 60 minutos", pickupTime: "20 a 30 minutos" };
...
export async function PUT(request: Request) {
  ...
  const { getDb } = await import("../../../db");
  const { storeSettings } = await import("../../../db/schema");
  const data = (await request.json()) as { isOpen?: boolean; deliveryTime?: string; pickupTime?: string };
  const deliveryTime = data.deliveryTime?.trim() ?? "";
  const pickupTime = data.pickupTime?.trim() ?? "";
  if (typeof data.isOpen !== "boolean" || !deliveryTime || !pickupTime || deliveryTime.length > 60 || pickupTime.length > 60) {
    return Response.json({ error: "Configuração inválida" }, { status: 400 });
  }
  const db = getDb();
  const [settings] = await db.insert(storeSettings).values({ id: 1, isOpen: data.isOpen, deliveryTime, pickupTime })
    .onConflictDoUpdate({ target: storeSettings.id, set: { isOpen: data.isOpen, deliveryTime, pickupTime, updatedAt: sql`CURRENT_TIMESTAMP` } })
    .returning();
